...
utils.inherits(WorkbookXform, BaseXform, {
  ...
  prepare: function(model) {
    ...
    model.sheets.forEach(function(sheet) {
      ...
      // if (sheet.pageSetup && sheet.pageSetup.printTitles && sheet.pageSetup.printTitles.length) {
      //   sheet.pageSetup.printTitles.forEach(function(printTitle) {
      //     const titlesRange = printTitle.split(':');
      //     const definedName = {
      //         name: '_xlnm.Print_Titles',
      //         ranges: ['\'' + sheet.name + '\'!$' + titlesRange[0] + ':$' + titlesRange[1]],
      //         localSheetId: index
      //     };
      //     printAreas.push(definedName);
      //   });
      ...
      if (sheet.pageSetup && sheet.pageSetup.printTitles && sheet.pageSetup.printTitles.length) {
        ...
        sheet.pageSetup.printTitles.forEach(function(printTitle) {
          const titlesRange = printTitle.split(':');
          ranges.push('\'' + sheet.name + '\'!$' + titlesRange[0] + ':$' + titlesRange[1]);
        });
    ...
    _.each(model.definedNames, function(definedName) {
      if (definedName.name === '_xlnm.Print_Area') {
        ...
        if (worksheet) {
          ...
          if (!worksheet.pageSetup.printTitles) {
            worksheet.pageSetup.printTitles = [];
          }
          const longRange = definedName.ranges[0].split('!');
          console.log('LONG RANGE', longRange);
          console.log('definedName.RANGES', definedName.ranges);
          const range = longRange[longRange.length - 1];
          console.log('range', range);
          worksheet.pageSetup.printTitles.push(range);
        }
      } else {
