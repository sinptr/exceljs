...
utils.inherits(HeaderFooterXform, BaseXform, {
  ...
  render: function render(xmlStream, model) {
    if (model) {
      ...
      if (model.header && model.header.odd) {
        this.map.oddHeader.render(xmlStream, this._buildRawString(model.header.odd));
      }
      if (model.header && model.header.even) {
        this.map.evenHeader.render(xmlStream, this._buildRawString(model.header.even));
      }
      if (model.header && model.header.first) {
        this.map.firstHeader.render(xmlStream, this._buildRawString(model.header.first));
      }
      if (model.footer && model.footer.odd) {
        this.map.oddFooter.render(xmlStream, this._buildRawString(model.footer.odd));
      }
      if (model.footer && model.footer.even) {
        this.map.evenFooter.render(xmlStream, this._buildRawString(model.footer.even));
      }
      if (model.footer && model.footer.first) {
        this.map.firstFooter.render(xmlStream, this._buildRawString(model.footer.first));
      }
      xmlStream.closeNode();
    }
  },
  ...
  _addAttrs: function _addAttrs(xmlStream, model) {
    if ((model.header && !_.isEqual(model.header.odd, model.header.even)) ||
      (model.footer && !_.isEqual(model.footer.odd, model.footer.even))) {
  ...
  _buildRawString: function _buildRawString(config) {
    if (config.raw) {
      return config.raw;
    }
    return '';
      ...
      case this.tag:
        this.model = {
          header: {
            ...
            even: this.map.evenHeader.model,
            first: this.map.firstHeader.model
          },
          footer: {
            odd: this.map.oddFooter.model,
            even: this.map.evenFooter.model,
            first: this.map.firstFooter.model
          }
        };
        return true;
      default:
